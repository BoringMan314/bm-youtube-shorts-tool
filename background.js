...
chrome.runtime.onMessage.addListener((msg, sender, sendResponse) => {
	...
	const filename =
		typeof payload.filename === 'string' ? payload.filename : 'youtube-shorts-record.webm';
	if (!shortUrl) {
	...
	(async () => {
		...
		try {
					...
					try {
						const [{ result }] = await chrome.scripting.executeScript({
							...
							func: async (rf) => {
								...
								for (let i = 0; i < 30 && stream.getVideoTracks().length === 0; i++)
									await waitMs(100);
			...
			safeSendToSenderTab({
				type: result && result.ok && result.dataUrl ? 'BM_BG_RECORD_DONE' : 'BM_BG_RECORD_ERROR',
				payload:
