...
`;
  ...
  function syncSpeedUiWithNativeLike() {
    ...
    const stylableBtns = btns.filter(
      (btn) =>
        btn instanceof HTMLElement && !btn.classList.contains("yts-recording-active")
    );
  ...
  function findRemixInner() {
    ...
    if (likeRow && likeRow.parentElement) {
      ...
      if (actionRows.length) {
        ...
        const rowsAfterLike =
          likeIdx >= 0 ? actionRows.slice(likeIdx + 1) : actionRows;
        const picked =
  ...
  function adjustVolumeBy(delta) {
    ...
    const syncNativeVolumeUi = (nextPct) => {
      ...
      if (slider instanceof HTMLElement) slider.style.setProperty("--ytp-volume-ratio", String(pct / 100));
  ...
  /** 目前 Shorts 主 video（優先 is-active，否則以可見面積與置中加權挑選） */
  ...
  function pickBestMuxedFormat(formats) {
    ...
    source.sort((a, b) => {
      ...
      const abrA = Number(a && a.averageBitrate ? a.averageBitrate : a && a.bitrate ? a.bitrate : 0);
      const abrB = Number(b && b.averageBitrate ? b.averageBitrate : b && b.bitrate ? b.bitrate : 0);
  ...
  function extractRecordingUrlFromPlayerResponse() {
    ...
    for (const pr of responses) {
      ...
      const formats = Array.isArray(sd.formats) ? sd.formats : [];
      // 先取最高畫質 video（可能無音訊）；抓不到再取 muxed。
      const bestVideo = pickBestVideoFormat(adaptive);
      if (bestVideo) {
        const u = bestVideo.url || parseSignatureCipher(bestVideo.signatureCipher || bestVideo.cipher || "");
        if (u) return u;
      }
      const bestMuxed = pickBestMuxedFormat(formats);
      if (bestMuxed) {
        const u = bestMuxed.url || parseSignatureCipher(bestMuxed.signatureCipher || bestMuxed.cipher || "");
        if (u) return u;
      }
    }
    return "";
  ...
  function startRecorderFallback() {
    ...
    const recordUrl =
      watchVideo.currentSrc ||
      watchVideo.src ||
      extractRecordingUrlFromPlayerResponse() ||
      "";
    if (!recordUrl) {
      fail("no_record_url");
      return;
      ...
      try {
        ...
        const stop = () => {
          ...
          if (recordingSession && recordingSession.stop === stop) {
            if (recordingSession.progressTimerId)
              clearInterval(recordingSession.progressTimerId);
  ...
  function updateDownloadRecordingUi(progressRatio, active) {
    if (!(downloadBtnEl instanceof HTMLElement) || !downloadBtnEl.isConnected) {
      ...
      const fallbackPct = fallbackBtn
        ? fallbackBtn.querySelector(".yts-record-percent")
        : null;
  ...
  function triggerVideoDownload() {
    ...
    const begin = async () => {
      ...
      const progressTimerId = setInterval(() => {
        if (!recordingSession) return;
        // Shorts 有時會自動循環，接近片尾就主動停錄，避免錄到下一輪。
  ...
  function stopManualRecording() {
    ...
    try {
      if (
        manualRecordSession.recorder &&
        manualRecordSession.recorder.state !== "inactive"
      ) {
  ...
  function buildScreenshotFilename() {
    ...
    const v = getActiveShortsVideo();
    const sec =
      v && Number.isFinite(v.currentTime) ? Math.max(0, Math.floor(v.currentTime)) : 0;
  ...
  function triggerFrameScreenshot() {
    ...
    chrome.runtime.sendMessage(
      ...
      (resp) => {
        ...
        if (!resp || !resp.ok) {
          console.warn("[BM Shorts Toolbox]", resp && resp.error ? resp.error : t("screenshotFailed"));
        }
      }
    );
  }
  ...
  function removeExternal3xWidgets() {
    ...
    all.forEach((el) => {
      ...
      // 工具箱版本一定有面板；舊 3x 只有單一倍速按鈕，移除以避免共存衝突。
